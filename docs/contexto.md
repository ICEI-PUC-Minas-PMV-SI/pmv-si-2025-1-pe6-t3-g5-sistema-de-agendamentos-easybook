...
|RF-002| Serviço de Notificações | BAIXA | |
|RF-003| O sistema deve executrar gestão de usuários, editar dados de perfil como nome, email, senha, descrição do perfil (fornecedores), excluir ou desativar perfil |  ALTA | Gabriel, Larissa |
|RF-004| Gestão de serviços (consulta, inclusão e exclusão) | BAIXA | Jean, Lincoln |
